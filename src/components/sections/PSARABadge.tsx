...
export default function PSARABadge({ certification, labels, href, className }: PSARABadgeProps) {
  ...
  const badge = (
    <Badge variant="outline" className={cn("h-auto gap-1.5 whitespace-normal border-border px-3 py-1.5 text-left text-xs", !isVerified && "text-muted-foreground", className)}>
      <ShieldCheck size={14} strokeWidth={1.5} aria-hidden="true" />
      <span>{label}</span>
    </Badge>
  );

  return href ? <Link to={href} aria-label={label}>{badge}</Link> : badge;
}
