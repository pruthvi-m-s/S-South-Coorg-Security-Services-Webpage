...
export default function Seo() {
  ...
  useEffect(() => {
    ...
    const imageUrl = absoluteUrl(
      meta.ogImage?.src ?? SEO_DEFAULTS.ogImage,
    );
    ...
    document.head
      .querySelectorAll(
        'script[data-sscss-schema="true"]',
      )
      .forEach((element) => element.remove());

    schemasForPath(pathname).forEach(
      (schema, index) => {
        const script =
          document.createElement("script");

        script.type = "application/ld+json";
        script.dataset.sscssSchema = "true";
        script.id = `sscss-schema-${index}`;
        script.textContent =
